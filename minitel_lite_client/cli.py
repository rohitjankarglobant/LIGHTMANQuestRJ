#!/usr/bin/env python3
"""
MiniTel-Lite Client CLI

This script connects to a MiniTel-Lite server, exchanges protocol messages,
retrieves an emergency override code, and optionally records the session.
"""
...
def run_client(
    ...
    client_class: Type[MiniTelLiteClient] = MiniTelLiteClient,
    protocol_class: Type[MiniTelLiteProtocol] = MiniTelLiteProtocol,
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        client_class: Client class to use (default: MiniTelLiteClient).
                      Can be replaced with a mock in tests.
        protocol_class: Protocol class to use (default: MiniTelLiteProtocol).
                        Can also be mocked in tests.
    """
    ...
    try:
        # Create and connect the client (mockable in tests)
        client = client_class(args.host, args.port, use_tls=not args.no_tls)
        ...
        if final_dump_response.get("status") == "success" and final_dump_response.get("data"):
